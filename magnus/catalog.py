"""
Magnus core service - Catalog.

get_run_log_store - Returns the instance of the run log store used by run
is_catalog_out_of_sync - return True if a particular catalog object differs from the one versioned already.

BaseCatalog - The base catalog class definition.
DummyCatalog - A dummy catalog implementation that does nothing.
FileSystemCatalog - A catalog implementation using the file system
"""
import logging
...
class BaseCatalog:
    ...
    Note: As a general guideline, do not extract anything from the config to set class level attributes.
          ...
          Access config properties using getters of the class.
    """
    service_name = None
...
class DummyCatalog(BaseCatalog):
    ...
    def get(self, name: str, run_id: str, compute_data_folder=None, **kwargs) -> List[object]:
        """
        Does nothing as it is a dummy catalog handler
        """
        logger.info('Using a dummy catalog, doing nothing in get')
        return []
    ...
    def put(self, name: str, run_id: str, compute_data_folder=None, synced_catalogs=None, **kwargs) -> List[object]:
        ...
        Does nothing as it is a dummy catalog handler.
        """
        logger.info('Using a dummy catalog, doing nothing in put')
        return []

    def sync_between_runs(self, previous_run_id: str, run_id: str):
        """
        Does nothing as it a dummy catalog hander.
        """
        logger.info('Using a dummy catalog, doing nothing while sync between runs')
        ...
    ...
    def sync_between_runs(self, previous_run_id: str, run_id: str):
        ...
        if not utils.does_dir_exist(catalog_location / previous_run_id):
            msg = (
                ...
                'Note: Please provision the catalog objects generated by the previous run in the same catalog location'
