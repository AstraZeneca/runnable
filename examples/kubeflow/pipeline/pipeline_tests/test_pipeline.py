...
def test_pipeline():
    ...
    with runner.isolated_filesystem():
        ...
        runner.invoke(clean_cli, ["--in-path", "raw_text.data", "--out-path", "clean_text.data"])
        ...
        runner.invoke(spacy_cli, ["--in-path", "clean_text.data", "--out-path", "tokenized_text.data"])

        with open("tokenized_text.data", "rb") as f:
            tokenized_arr = dill.load(f)
            expected_array = np.array([["hello", "this", "be", "a", "test"], ["another", "sentence", "to", "process"]])
