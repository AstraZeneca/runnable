...
class SimpleVisualizer:
    ...
    def generate_html_timeline(
        ...
        executed_steps = [step for step in self.timeline if step.start_time]
        if executed_steps:
            earliest = min(step.start_time for step in executed_steps)
            latest = max(step.end_time for step in executed_steps if step.end_time)
            total_duration_ms = (
                (latest - earliest).total_seconds() * 1000 if latest else 1
            )
        else:
            total_duration_ms = 1
...
<body>
    ...
    def _generate_html_timeline_rows(self, total_duration_ms: float) -> str:
        """Generate HTML rows for the timeline display."""
        html_parts = []
        ...
        for step in self.timeline:
            ...
            if composite and composite != current_composite:
                ...
                html_parts.append(f"""
                    <div class="step-row composite-header expandable" data-composite="{composite}">
                        ...
                        <div class="timeline-bar-container"></div>
            ...
            if step.start_time and step.duration_ms > 0:
                # Calculate bar position and width (simplified for now)
                bar_width = (
                    max(1, (step.duration_ms / total_duration_ms) * 100)
            ...
            # These were for tooltip functionality which has been removed

            # Build parameter display - compact horizontal format
            param_info = []

            # Input parameters - compact horizontal display
            if step.input_params:
            ...
            # Output parameters - compact horizontal display
            if step.output_params:
            ...
            # Catalog operations - compact horizontal display
            if step.catalog_ops.get("put") or step.catalog_ops.get("get"):
                catalog_items = []
                if step.catalog_ops.get("put"):
                    catalog_items.extend(
            ...
            step_id = f"step-{step.internal_name.replace('.', '-')}-{step.start_time.isoformat() if step.start_time else 'no-time'}"
            ...
            html_parts.append(f"""
                <div class="step-row">
                    <div class="step-info step-level-{step.level}">
                        ...
                        {f'<div style="color: #64748b; font-size: 0.875rem; margin-top: 0.25rem;"><strong>{step.command_type.upper()}:</strong> {step.command[:60]}{"..." if len(step.command) > 60 else ""}</div>' if step.command else ''}
                    ...
                    <div class="duration-text {duration_class}">
    ...
    def _generate_html_summary(self) -> str:
        ...
        <div class="summary-grid">
            ...
            <div class="summary-card">
                <div class="summary-number duration-slow">{slowest_step.duration_ms:.1f}ms</div>
