...
def test_task_node_parse_from_config_seperates_task_from_node_confifg(mocker, monkeypatch):
    ...
    task_node = nodes.TaskNode.parse_from_config({**node_config, **command_config}, internal_name="test")
...
def test_parallel_node_parse_from_config_creates_sub_graph(mocker, monkeypatch):
    ...
    parallel_node = nodes.ParallelNode.parse_from_config(config=config, internal_name="parent")
...
def test_parallel_node_parse_from_config_raises_exception_if_no_branches(mocker, monkeypatch):
    ...
    with pytest.raises(Exception, match="A parallel node should have branches"):
        _ = nodes.ParallelNode.parse_from_config(config=config, internal_name="parent")
...
def test_map_node_parse_from_config_raises_exception_if_no_branch(mocker, monkeypatch):
    ...
    with pytest.raises(Exception, match="A map node should have a branch"):
        _ = nodes.MapNode.parse_from_config(config=config, internal_name="parent")
...
def test_map_node_parse_from_config_calls_create_graph(mocker, monkeypatch):
    ...
    monkeypatch.setattr(nodes, "create_graph", mock_create_graph)
    map_node = nodes.MapNode.parse_from_config(config=config, internal_name="parent")
...
def test_dag_node_parse_config_raises_exception_if_dag_definition_is_not_part_of_config(mocker, monkeypatch):
    ...
    with pytest.raises(Exception, match="No dag definition found in"):
        _ = nodes.DagNode.parse_from_config(config=config, internal_name="parent")
...
def test_parse_config_calls_create_graph(mocker, monkeypatch):
    ...
    dag_node = nodes.DagNode.parse_from_config(config=config, internal_name="parent")
