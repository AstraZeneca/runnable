...
from magnus.extensions.executor.local_container import implementation


def test_configure_for_traversal_populates_volumes(mocker, monkeypatch):
    mock_local_container = mocker.MagicMock()
    monkeypatch.setattr(implementation, "LocalContainerComputeFileSystemRunLogstore", mock_local_container)
    ...
    test_integration = implementation.LocalContainerComputeFileSystemCatalog(mock_executor, mock_fs_catalog)
