...
import magnus.extensions.catalog.file_system.implementation as implementation
...
def test_file_system_catalog_get_copies_files_from_catalog_to_compute_folder_with_all(mocker, monkeypatch):
    ...
    mocker.patch(
        "magnus.catalog.BaseCatalog._context",
    ...
    with tempfile.TemporaryDirectory() as catalog_location:
        with tempfile.TemporaryDirectory(dir=".") as compute_folder:
            ...
            run_id = "testing"
            implementation.Path(catalog_location_path / run_id / compute_folder).mkdir(parents=True)
            with open(implementation.Path(catalog_location) / run_id / compute_folder / "catalog_file", "w") as fw:
                fw.write("hello")
...
def test_file_system_catalog_put_copies_files_from_compute_folder_to_catalog_if_synced_changed_all(mocker, monkeypatch):
    monkeypatch.setattr(implementation, "is_catalog_out_of_sync", mocker.MagicMock(return_value=True))
    mock_run_store = mocker.MagicMock()
    mock_context = mocker.MagicMock()
    mock_context.run_log_store = mock_run_store
...
def test_file_system_catalog_put_copies_files_from_compute_folder_to_catalog_if_synced_true(mocker, monkeypatch):
    monkeypatch.setattr(implementation, "is_catalog_out_of_sync", mocker.MagicMock(return_value=False))
    mock_run_store = mocker.MagicMock()
    mock_context = mocker.MagicMock()
    mock_context.run_log_store = mock_run_store
...
def test_file_system_catalog_put_creates_catalog_location_using_run_id(monkeypatch, mocker):
    ...
    mock_safe_make_dir.assert_called_once_with(implementation.Path("this_location") / "dummy_run_id")
