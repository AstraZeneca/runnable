...
def test_python_execute_command_raises_exception_if_function_fails(mocker, monkeypatch):
    ...
    monkeypatch.setattr(nodes.utils, 'get_module_and_func_names',
                        mocker.MagicMock(return_value=('idk', 'func')))
    monkeypatch.setattr(nodes.importlib, 'import_module',
                        mocker.MagicMock(return_value=DummyModule()))

    monkeypatch.setattr(nodes.utils, 'filter_arguments_for_func',
                        mocker.MagicMock(return_value={'a': 1}))
...
def test_python_execute_command_sends_no_mapped_variable_if_not_present_in_signature(mocker, monkeypatch):
    ...
    py_exec = nodes.PythonExecutionType()
    py_exec.execute_command(command='test', map_variable={
                            'map_name': 'map_value'})
    dummy_func.assert_called_once_with(a=1)
...
def test_base_node_get_internal_name_from_command_name_replaces_character_with_whitespace():
    ...
    assert nodes.BaseNode.get_internal_name_from_command_name(
        'test%') == 'test '
...
def test_base_node_get_step_log_name_returns_map_modified_internal_name_if_map_variable_multiple():
    ...
    assert node.get_step_log_name(
        map_variable={'map_key': 'a', 'map_key1': 'b'}) == 'test.a.step.b'
...
def test_base_node_get_branch_log_name_returns_map_modified_internal_name_if_map_variable_multiple():
    ...
    assert node.get_branch_log_name(
        map_variable={'map_key': 'a', 'map_key1': 'b'}) == 'test.a.step.b'
...
def test_base_node_get_on_failure_node_returns_node_name_if_defined():
    node = nodes.BaseNode(name='test', internal_name='test',  config={
                          'on_failure': 'fail'}, execution_type=None)
...
def test_base_node_get_next_node_returns_config_next():
    node = nodes.BaseNode(name='test', internal_name='test',  config={
                          'next': 'IamNext'}, execution_type=None)
...
def test_validate_node_raises_exception_for_unspecified_node(mocker, monkeypatch):
    ...
    monkeypatch.setattr(nodes.utils, 'load_yaml',
                        mocker.MagicMock(return_value=dummy_specs))
...
def test_validate_node_sends_messages_if_required_are_not_present(mocker, monkeypatch):
    ...
    node = nodes.BaseNode(name='test', internal_name='test',  config={
                          'dummy_required1': True}, execution_type=None)
...
def test_validate_node_sends_empty_if_required_present(mocker, monkeypatch):
    ...
    node = nodes.BaseNode(name='test', internal_name='test',  config={
                          'dummy_required': True}, execution_type=None)
...
def test_task_node_mocks_if_mock_is_true(mocker, monkeypatch):
    ...
    mock_executor.run_log_store.create_attempt_log = mocker.MagicMock(
        return_value=mock_attempt_log)

    task_node = nodes.TaskNode(name='test', internal_name='test', config={
...
def test_task_node_sends_map_variable_if_sent_to_execution(mocker, monkeypatch):
    ...
    assert mock_attempt_log.status == defaults.SUCCESS
    mock_execution_command.assert_called_once_with(
        'nocommand', map_variable={'map_key': 'a'})
...
def test_fail_node_sets_branch_log_fail(mocker, monkeypatch):
    ...
    mock_executor.run_log_store.create_attempt_log = mocker.MagicMock(
        return_value=mock_attempt_log)
    mock_executor.run_log_store.get_branch_log = mocker.MagicMock(
        return_value=mock_branch_log)
...
def test_parallel_node_raises_exception_for_empty_branches():
    with pytest.raises(Exception):
        nodes.ParallelNode(name='test', internal_name='test', config={
                           'branches': {}}, execution_type='python')
...
def test_parallel_node_get_sub_graphs_creates_graphs(mocker, monkeypatch):
    ...
    node = nodes.ParallelNode(
        name='test', internal_name='test', config=parallel_config, execution_type='python')
...
def test_parallel_node_get_branch_by_name_raises_exception_if_branch_not_found(mocker, monkeypatch):
    monkeypatch.setattr(nodes.ParallelNode,
                        'get_sub_graphs', mocker.MagicMock())

    node = nodes.ParallelNode(
        name='test', internal_name='test', config={}, execution_type='python')
...
def test_nodes_map_node_raises_exception_if_config_not_have_iterate_on():
    with pytest.raises(Exception):
        nodes.MapNode(name='test', internal_name='test',
                      config={}, execution_type='test')


def test_nodes_map_node_raises_exception_if_config_not_have_iterate_as():
    with pytest.raises(Exception):
        nodes.MapNode(name='test', internal_name='test', config={
                      'iterate_on': 'y'}, execution_type='test')
...
def test_nodes_dag_node_raises_exception_if_dag_definition_is_not_present():
    with pytest.raises(Exception):
        nodes.DagNode(name='test', internal_name='test',
                      config={}, execution_type='test')
...
def test_node_dag_node_get_sub_graph_raises_exception_if_dag_block_not_present(mocker, monkeypatch):
    ...
    with pytest.raises(Exception):
        nodes.DagNode(name='test', internal_name='test', config={
                      'dag_definition': 'a'}, execution_type='test')
...
def test_nodes_dag_node_get_sub_graph_calls_create_graph_with_correct_parameters(mocker, monkeypatch):
    ...
    _ = nodes.DagNode(name='test', internal_name='test', config={
                      'dag_definition': 'a'}, execution_type='test')

    mock_create_graph.assert_called_once_with(
        'a', internal_branch_name='test.' + defaults.DAG_BRANCH_NAME)


def test_nodes_dag_node_get_branch_by_name_raises_exception_if_branch_name_is_invalid(mocker, monkeypatch):
    monkeypatch.setattr(nodes.DagNode, 'get_sub_graph',
                        mocker.MagicMock(return_value='branch'))

    node = nodes.DagNode(name='test', internal_name='test', config={
                         'dag_definition': 'a'}, execution_type='test')
...
def test_nodes_dag_node_get_branch_by_name_returns_if_branch_name_is_valid(mocker, monkeypatch):
    ...
    assert node.get_branch_by_name(
        'test.' + defaults.DAG_BRANCH_NAME) == 'branch'
...
def test_nodes_as_is_node_defaults_render_string_to_none():
    node = nodes.AsISNode(name='test', internal_name='test',
                          config={}, execution_type='test')
...
def test_nodes_as_is_node_accepts_what_is_given():
    node = nodes.AsISNode(name='test', internal_name='test', config={
                          'render_string': 'test'}, execution_type='test')
