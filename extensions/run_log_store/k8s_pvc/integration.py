...
class ArgoCompute(BaseIntegration):
    ...
    def configure_for_traversal(self, **kwargs):
        from runnable.extensions.executor.argo.implementation import (
            ArgoExecutor,
            UserVolumeMounts,
        )
        from runnable.extensions.run_log_store.k8s_pvc.implementation import (
            K8PersistentVolumeRunLogstore,
        )
        ...
        volume_mount = UserVolumeMounts(
            name=self.service.persistent_volume_name, mount_path=self.service.mount_path
        )
